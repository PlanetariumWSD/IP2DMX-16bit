...
IPAddress ip(1, 1, 1, 42); // Local Area Network Address Assigned
//byte ip[] = {1,1,1,42};
unsigned int localPort = 8888; // local port to listen on

//IPAddress ip(1, 1, 1, 40); // Local Area Network Address Assigned
byte ipMulti[] = {1,1,1,40};
unsigned int portMulti = 8889; // local port to listen on

EthernetUDP Udp;    // An EthernetUDP instance to let us send and receive packets over UDP
//EthernetUDP UdpMulti;
...
void setup() {  //-------------------------------------------------------------------------------
  ...
  //Udp.beginMulticast(IPAddressMulti);
  Udp.begin(localPort);
  Udp.beginMulticast(ipMulti, localPort);
  //UdpMulti.begin(ipMulti, localPortMulti);
