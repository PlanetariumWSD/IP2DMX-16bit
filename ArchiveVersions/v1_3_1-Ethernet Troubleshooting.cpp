...
// v1_3_1
// Added Ethernet troubleshooting (Link Status & UDP reset on switch) and serialized MAC addresses
//--------------------------------------------------------------------------------
// Wall House Lights Switch durations
uint32_t neglectDuration = 16200000;       // milliseconds, 4.5 hours = 16200000 ms / 5000
uint32_t neglectWarningDuration = 120000;  // 2 minutes = 120000 ms / 5000
uint32_t neglectSafetyDuration = 300000;   // 5 minutes = 300000 ms / 5000

IPAddress ip(192, 168, 2, 246);                     // Local Area Network Address Assigned (Bench 1, 1, 1, 40)
byte mac[] = {0xDE, 0xAD, 0xBE, 0xEF, 0x00, 0x01};  // Physcial Hardware Address of Unique Entity
...
const int relayListen = A1;  // Pin to activate relay that engages listening pin WallSwtich.
const int relayInt = A2;     // Pin to activate relay that interupts Chromacove house lights.
bool switchStatus = 0;
Timer timer;
...
void setup() {
  ...
  Udp.begin(localPort);
  timer.reset();
  pinMode(relayListen, OUTPUT);
  pinMode(relayInt, OUTPUT);
  pinMode(cmdRcvLED, OUTPUT);
  pinMode(ErrorLED, OUTPUT);
  digitalWrite(relayListen, HIGH);
};
// =================================================================================
void loop() {
